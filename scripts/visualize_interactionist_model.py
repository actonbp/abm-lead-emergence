...
"""Visualize the dynamics of the interactionist leadership emergence model."""
...
def run_and_visualize_interactionist_model(n_runs=20, n_steps=100):
    """Run and visualize the interactionist model with best parameters."""
    # Best parameters from sweep
    best_params = {
        'n_agents': 6,                     # Moderate group size
        'schema_dimensions': 3,            # Rich trait space
        'match_algorithm': 'average',      # Average matching for smoother dynamics
        'match_threshold': 0.635,          # Moderately selective
        'success_boost': 7.99,             # Strong reinforcement
        'failure_penalty': 2.72,           # Moderate penalty
        'rejection_penalty': 5.11,         # Significant rejection cost
        'passivity_penalty': 3.26,         # Moderate passivity cost
        'switch_step': 23,                 # Early-mid transition
        'identity_transition_steps': 6,     # Gradual transition
        
        # Fixed parameters
        'characteristic_distribution': 'normal',
        'ilt_distribution': 'normal',
        'distribution_mean': 50.0,         # Center of 0-100 scale
        'distribution_std': 15.0,          # Reasonable spread
    ...
    for run in range(n_runs):
        ...
        model_params = InteractionistParameters(**best_params)
        model = InteractionistModel(model_params)
    ...
    ax1.fill_between(time_points, 
                     ...
                     mean_entropy + std_entropy,
                     alpha=0.2)
    # Add vertical line for identity switch
    ax1.axvline(x=best_params['switch_step'], color='r', linestyle='--',
                label=f'Identity Switch (t={best_params["switch_step"]})')
    ax1.set_title('Hierarchy Entropy Over Time (Interactionist Model)')
    ...
    ax2.fill_between(time_points,
                     ...
                     alpha=0.2)
    # Add vertical line for identity switch
    ax2.axvline(x=best_params['switch_step'], color='r', linestyle='--',
                label=f'Identity Switch (t={best_params["switch_step"]})')
    ax2.set_title('Perception Agreement Over Time (Interactionist Model)')
    ...
    ax3.fill_between(time_points,
                     ...
                     alpha=0.2)
    # Add vertical line for identity switch
    ax3.axvline(x=best_params['switch_step'], color='r', linestyle='--',
                label=f'Identity Switch (t={best_params["switch_step"]})')
    ax3.set_title('Leadership Differentiation Over Time (Interactionist Model)')
    ...
    param_text = (
        ...
        f"Reject={best_params['rejection_penalty']:.2f}, "
        f"Passive={best_params['passivity_penalty']:.2f}\n"
        f"Switch={best_params['switch_step']}, "
        f"Trans.Steps={best_params['identity_transition_steps']}"
    )
    ...
    filename = f'outputs/plots/social_interactionist_model_metrics_{timestamp}.png'
    plt.savefig(filename, dpi=300, bbox_inches='tight')
    print(f"Plot saved to: {filename}")
    plt.close()

if __name__ == "__main__":
    run_and_visualize_interactionist_model() 